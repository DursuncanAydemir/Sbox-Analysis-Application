...
namespace SboxAnalysis.Controllers
{
    public class HomeController : Controller
    {
        ...
        private LinearResultModel CalculateLineerProbability(byte[] sboxArr)
        {
            ...
            var nonLinMaxVal = 8 - linMax;
            var nonLinMinVal = 8 - linMin;
            var nonLinAvgVal = 8 - Convert.ToInt32(avgLin);
            ...
            linearResult.MaxNonLinearProbability = Math.Abs(Math.Round(maxNonLin, 4));
            linearResult.MinNonLinearProbability = Math.Abs(Math.Round(minNonLin, 4));
            linearResult.AvgNonLinearProbability = Math.Abs(Math.Round(avgNonLin, 4));
            ...
            linearResult.SacMax = Math.Round((sacMax / 3), 4);
            linearResult.SacMin = Math.Round((sacMin / 3), 4);

            var bıc = sbox_bic(sboxArr, 3, 3);

            linearResult.BıcSacAvg = Math.Abs(Math.Round((bıc - sacAvg), 4));
            linearResult.BıcSacMin = Math.Abs(Math.Round((bıc - sacMin), 4));
            linearResult.BıcSacMax = Math.Abs(Math.Round((bıc - sacMax), 4));
            return linearResult;
        }
